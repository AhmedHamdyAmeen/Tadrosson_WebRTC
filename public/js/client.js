...
/**
 * Tadrosoon P2P - Client component
 *
...
const avatarApiUrl = 'https://eu.ui-avatars.com/api'; // Avatar Creator
...
const aboutImg = '../images/Tadrosoon-logo.png';
const imgFeedback = '../images/feedback.png';
const forbiddenImg = '../images/forbidden.png';
const avatarImg = '../images/Tadrosoon-logo.png';
...
let recStartTime;
let recElapsedTime;
let TadrosoonTheme = 'dark'; // dark - grey ...
let TadrosoonBtnsBar = 'vertical'; // vertical - horizontal
...
function getHtmlElementsById() {
    ...
    themeSelect = getId('TadrosoonTheme');
    videoObjFitSelect = getId('videoObjFitSelect');
    btnsBarSelect = getId('TadrosoonBtnsBar');
            ...
            switch (event.channel.label) {
                case 'Tadrosoon_chat_channel':
                    try {
                        ...
                        console.error('Tadrosoon_chat_channel', err);
                    }
                    break;
                case 'Tadrosoon_file_sharing_channel':
                    try {
                        let dataFile = msg.data;
                        handleDataChannelFileSharing(dataFile);
                    } catch (err) {
                        console.error('Tadrosoon_file_sharing_channel', err);
                    }
                    break;
            }
        };
                    ...
                    .then((local_description) => {
                        ...
                        peerConnections[peer_id]
                            ...
                            .then(() => {
                                ...
                                // https://github.com/miroslavpejic85/Tadrosoon/issues/110
...
/**
 * Set Tadrosoon theme | dark | grey | ...
 * @param {string} theme type
 */
function setTheme(theme) {
    if (!theme) return;

    TadrosoonTheme = theme;
    switch (TadrosoonTheme) {
        case 'dark':
            // dark theme
    ...
    setButtonsBarPosition(TadrosoonBtnsBar);
}
...
function setButtonsBarPosition(position) {
    ...
    TadrosoonBtnsBar = position;
    switch (TadrosoonBtnsBar) {
        case 'vertical':
...
async function initEnumerateDevices() {
    ...
    if (!useAudio && !useVideo) {
        ...
        await Swal.fire({
            ...
            if (result.isConfirmed) {
                openURL('/newcall'); // back to newcall page
            }
        });
    }
}
...
function setMyWhiteboardBtn() {
    ...
    whiteboardGhostButton.addEventListener('click', (e) => {
        ...
        //wbIsBgTransparent ? wbCanvasBackgroundColor('rgba(0, 0, 0, 0.100)'): setTheme(TadrosoonTheme);
    });
}
...
function shareRoomMeetingURL(checkScreen = false) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            let message = {
                email: '',
                subject: 'Please join our Tadrosoon Video Chat Meeting',
...
function createChatDataChannel(peer_id) {
    chatDataChannels[peer_id] = peerConnections[peer_id].createDataChannel('Tadrosoon_chat_channel');
...
function createFileSharingDataChannel(peer_id) {
    fileDataChannels[peer_id] = peerConnections[peer_id].createDataChannel('Tadrosoon_file_sharing_channel');
...
function selectFileToShare(peer_id, broadcast = false) {
    ...
    Swal.fire({
        allowOutsideClick: false,
        background: swalBackground,
        imageAlt: 'Tadrosoon-file-sharing',
    ...
    if (isImageURL(incomingFileInfo.file.fileName)) {
        ...
        reader.onload = (e) => {
            Swal.fire({
                ...
                imageAlt: 'Tadrosoon-file-img-download',
                showDenyButton: true,
        ...
        Swal.fire({
            allowOutsideClick: false,
            background: swalBackground,
            imageAlt: 'Tadrosoon-file-download',
            imageUrl: fileSharingImg,
            position: 'center',
            title: 'Received file',
            text: incomingFileInfo.file.fileName + ' size ' + bytesToSize(incomingFileInfo.file.fileSize),
...
function showAbout() {
    ...
    Swal.fire({
        ...
        imageAlt: 'Tadrosoon-about',
        imageUrl: aboutImg,
        html: `
        <br/>
        <div id="about">
            <b><a href="https://github.com/miroslavpejic85/Tadrosoon" class="umami--click--github" target="_blank">Open Source</a></b> project
            <br/><br/>
...
function leaveFeedback() {
    Swal.fire({
        ...
        text: 'Do you want to rate your Tadrosoon experience?',
        confirmButtonText: `Yes`,
        denyButtonText: `No`,
        showClass: {
            popup: 'animate__animated animate__fadeInDown',
